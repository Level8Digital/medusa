@extends('layouts.layout3')

@section('title', 'Insights')

@section('styles')

@endsection

@section('content')

<!-- Header -->
<header class="header header-inverse bg-fixed" style="background-image: url(assets/img/greek-gods.jpg)" data-overlay="8">
  <div class="container text-center">

    <div class="row">
      <div class="col-12 col-lg-8 offset-lg-2">

        <h1>Trading Insights</h1>
        <p class="fs-20 opacity-70">Articles designed to take your trading to the next level.</p>

      </div>
    </div>

  </div>
</header>

<!-- Main container -->
<main class="main-content">


  <div class="row">
    <div class="col-12 col-lg-4 offset-lg-4">

      <!--
      |‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒
      | Article
      |‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒
      !-->
      <article class="mt-90">
        <header class="text-center mb-40">
          <h3><a href="blog-single.html"><em>Pro Insights Part 1:</em> Probability & Random Distribution</a></h3>
          <div class="link-color-default fs-12">
            <a href="#">News</a>,
            <time>April 23, 2022</time>
          </div>
        </header>

        <div class="text-center">
          <a href="blog-single.html"><img class="rounded" src="{{ url('assets/img/probability.jpg') }}" alt="..."></a>
        </div>


        <div class="card-block">

          <p class="text-justify">Together. Great. So good was saying, that can't first let called air divide stars male isn't i. Herb third let may fourth divide. Greater gathering land you'll i their beast have. She'd form sea it wherein fowl, spirit creeping living. Likeness creepeth you hath heaven. Likeness, moveth fruitful behold. Open evening a air us behold. Saying above moving second a subdue likeness after also second.</p>

          <p class="text-center mt-40">
            <a class="btn btn-primary btn-round" href="blog-single.html">Read more</a>
          </p>

        </div>
      </article>
      <hr class="dark-hr">

      <!--
      |‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒
      | Article
      |‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒
      !-->
      <article class="mt-90">
        <header class="text-center mb-40">
          <h3><a href="blog-single.html"><em>Pro Insights Part 2:</em> Tipping The Odds</a></h3>
          <div class="link-color-default fs-12">
            <a href="#">News</a>,
            <time>April 23, 2022</time>
          </div>
        </header>

        <div class="text-center">
          <a href="blog-single.html"><img class="rounded" src="{{ url('assets/img/odds.jpg') }}" alt="..."></a>
        </div>


        <div class="card-block">

          <p class="text-justify">Together. Great. So good was saying, that can't first let called air divide stars male isn't i. Herb third let may fourth divide. Greater gathering land you'll i their beast have. She'd form sea it wherein fowl, spirit creeping living. Likeness creepeth you hath heaven. Likeness, moveth fruitful behold. Open evening a air us behold. Saying above moving second a subdue likeness after also second.</p>

          <p class="text-center mt-40">
            <a class="btn btn-primary btn-round" href="blog-single.html">Read more</a>
          </p>

        </div>
      </article>
      <hr class="dark-hr">

      <!--
      |‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒
      | Article
      |‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒
      !-->
      <article class="mt-90">
        <header class="text-center mb-40">
          <h3><a href="blog-single.html"><em>Pro Insights Part 3:</em> Shaping The Mind</a></h3>
          <div class="link-color-default fs-12">
            <a href="#">News</a>,
            <time>April 23, 2022</time>
          </div>
        </header>

        <div class="text-center">
          <a href="blog-single.html"><img class="rounded" src="{{ url('assets/img/mind.jpg') }}" alt="..."></a>
        </div>


        <div class="card-block">

          <p class="text-justify">Together. Great. So good was saying, that can't first let called air divide stars male isn't i. Herb third let may fourth divide. Greater gathering land you'll i their beast have. She'd form sea it wherein fowl, spirit creeping living. Likeness creepeth you hath heaven. Likeness, moveth fruitful behold. Open evening a air us behold. Saying above moving second a subdue likeness after also second.</p>

          <p class="text-center mt-40">
            <a class="btn btn-primary btn-round" href="blog-single.html">Read more</a>
          </p>

        </div>
      </article>
      <hr class="dark-hr">


    </div>
  </div>



</main>
<!-- END Main container -->

@endsection
